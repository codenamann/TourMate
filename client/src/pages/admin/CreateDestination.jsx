...
const CreateDestination = () => {
  ...
  useEffect(() => {
    const searchCitiesDebounced = async () => {
      if (!citySearch || citySearch.length < 2) {
        if (formData.stateId) {
          // If state is selected, show cities for that state
          const response = await getCitiesByState(formData.stateId)
          setCities(response.data)
        }
        return
  ...
  const handleSubmit = async (e) => {
    ...
    let highlights = []
    if (typeof formData.highlights === "string" && formData.highlights.trim() !== "") {
    ...
    // Prepare payload - remove stateId, ensure category is "destination", highlights is array
    const payload = {
      name: formData.name,
      cityId: formData.cityId,
      category: "destination", // Always "destination" for this form
    ...
    setLoading(true)
    try {
      await createDestination(payload, selectedFiles)
      addToast({
        title: "Success",
      ...
      addToast({
        title: "Error",
        description: error.response?.data?.message || "Failed to create destination",
        variant: "destructive"
      })
    } finally {
      setLoading(false)
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-foreground mb-6">Create New Destination</h1>
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Category is set to "destination" for this form
              </p>
            </div>
